  ...
  if (!order) {
    return (
      <Stack>
        ...
        <Text c="red">Sales order not found</Text>
      </Stack>
    );
  }
    ...
    confirmOrder.mutate(orderId, {
      onSuccess: () => {
        notifications.show({
          ...
          message: `Stock has been deducted for order #${orderId}`,
          color: 'green',
        });
        closeConfirm();
      },
    ...
    <Stack gap="lg">
      <Group justify="space-between">
        <Group>
          ...
          <Title order={2}>Sales Order #{order.id}</Title>
      ...
      >
        <Stack>
          <Text>
            Confirming this order will deduct stock for {order.items.length} item
            {order.items.length === 1 ? '' : 's'} and record{' '}
            {formatCurrency(total)} in revenue. This cannot be undone.
          </Text>
